    ...
    >
      <div className="sidebar-thumb" style={{ width: 36, height: 36, background: "#eee", marginRight: 12, display: "flex", alignItems: "center", justifyContent: "center" }}>
        {isVideo(blog.img_url) ? (
          <video src={blog.img_url} style={{ width: 28, height: 28, objectFit: "cover" }} muted autoPlay loop />
        ) : (
  ...
  const cards = filteredBlogs.map((blog, idx) => (
    <div key={idx} className="blog-card" onClick={() => setSelected(blogs.indexOf(blog))} style={{ cursor: "pointer" }}>
      <div className="blog-card-img" style={{ background: "#f4f4f4", height: 120, display: "flex", alignItems: "center", justifyContent: "center" }}>
        {blog.img_url ? (
          isVideo(blog.img_url) ? (
            <video src={blog.img_url} style={{ maxHeight: "100%", maxWidth: "100%" }} muted autoPlay loop />
          ) : (
      ...
      <div style={{ margin: "20px 0" }}>
        {isVideo(blogs[selected].img_url) ? (
          <video
            ...
            style={{ maxWidth: "500px", borderRadius: 8, marginLeft: 24, marginBottom: 108 }}
            controls
          />
        ) : (
          <img
            src={blogs[selected].img_url}
            alt=""
            style={{ maxWidth: 320, borderRadius: 8, float: "right", marginLeft: 24, marginBottom: 8 }}
          />
        )}
